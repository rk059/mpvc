    ...
    <section class="admin-panel" style="margin-top:24px">
      ...
      <div class="admin-list">
        <?php if (!$media): ?><p class="muted">No uploaded gallery media yet.</p><?php else: foreach ($media as $item): ?>
          <div class="admin-item"><div><strong><?= e((string)$item['caption']) ?></strong><small><?= !empty($item['is_default']) ? 'Default image' : 'Uploaded media' ?> · <?= e((string)$item['type']) ?> · <?= e(public_date((string)$item['created_at'])) ?></small></div><div class="admin-actions"><a href="../<?= e((string)$item['file']) ?>" target="_blank">Open</a><form method="post"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?= e((string)$item['id']) ?>"><button class="danger" type="submit">Remove</button></form></div></div>
